...
num_expected_xsum_articles = 226711
  ...
  with open(out_prefix + '.source', 'wt') as source_file, open(out_prefix + '.target', 'wt') as target_file:
    for idx,s in enumerate(tqdm(article_fnames)):
      ...
      if os.path.isfile(os.path.join(args.articles, s)):
        ...
        check_num_articles(args.articles, num_expected_xsum_articles)
...
def main():
  ...
  parser.add_argument('--xsum', default='../shared/xsum/')
  parser.add_argument('--articles', default='../shared/xsum/articles_tokenized/')
  parser.add_argument("--res", default='exp_test/xsum.tokenized')
  ...
  write_to_bin(args, "%s/all_test.txt" % args.xsum, os.path.join(args.res, "test"), doc_sep=args.sep)
  write_to_bin(args, "%s/all_validation.txt" % args.xsum, os.path.join(args.res, "valid"), doc_sep=args.sep)
  write_to_bin(args, "%s/all_train.txt" % args.xsum, os.path.join(args.res, "train"), doc_sep=args.sep)
